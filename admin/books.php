...
echo '<form method="post" class="bg-white border rounded p-4 mb-6 grid grid-cols-1 md:grid-cols-5 gap-2">';
...
echo '<button class="px-3 py-2 bg-blue-600 text-white rounded" title="INSERT INTO books(...) VALUES(...) (inside TRANSACTION)">Add Book</button>';
echo '</div></form>';

echo '<table class="w-full bg-white border rounded">';
echo '<tr class="text-left border-b"><th class="p-2">Title</th><th class="p-2">Category</th><th class="p-2">Actions</th></tr>';
foreach ($rows as $r) {
	echo '<tr class="border-b"><td class="p-2">' . e($r['title']) . '</td><td class="p-2">' . e($r['cat_name']) . '</td>';
	echo '<td class="p-2"><a class="text-red-600" title="DELETE FROM books WHERE book_id = ?" href="?action=delete&id=' . (int)$r['book_id'] . '" onclick="return confirm(\'Delete?\')">Delete</a></td></tr>';
}
echo '</table>';
